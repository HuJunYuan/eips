...
(function () {
    ...
    $.extend($.fn.validatebox.extensions, {
        ...
        val: function (target) {
            var dom = $(target);
            return dom.next().hasClass("textbox-value") ? dom.next().val() : dom.val();
        },
        rules: {
            //  只允许输入半角字符号或数字
            chaNum: {
    ...
    //$.extend($.fn.validatebox.defaults.val, $.fn.validatebox.extensions.val);
    $.fn.validatebox.defaults.val = function (target) {
        var dom = $(target);
        return dom.next().hasClass("textbox-value") ? dom.next().val() : dom.val();
    };
