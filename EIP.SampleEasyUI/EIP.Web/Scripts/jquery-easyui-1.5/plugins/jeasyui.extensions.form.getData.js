...
(function ($) {
    ...
    function validateExt(target) {
        var t = $(target), state = $.data(target, "form"), opts = state ? state.options : $.fn.form.defaults;
        if ($.fn.validatebox) {
            t.find('.validatebox-text:not(:disabled)').validatebox('validate');
        }
        if ($.fn.richtextbox && !opts.novalidate) {
            t.find('textarea.easyui-richtextbox:not(:disabled)').prev()
                .each(function () {
                    $(this).richtextbox("validate");
                });
        }
    ...
    var methods = $.fn.form.extensions.methods = {
        ...
        validate: function (jq) { return validateExt(jq[0]);}
    };
